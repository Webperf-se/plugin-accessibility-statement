...
export class HarAnalyzer {
    ...
    if (knowledgeData['compatible-word'] || knowledgeData['notification-function-link-url'] || knowledgeData['unreasonably-burdensome-accommodation-word']) {
        // Tidigare krävdes hela det juridiska ordet "tillgänglighetsredogörelse"
        // i h1/title. Det missar sidor med kortare rubriker som
        // "Tillgänglighet på webbplatsen" eller bara "Tillgänglighet",
        // vilket är vanligt på regions- och kommunsajter.
        //
        // Vi tillåter nu "tillgänglighet" som rotord. Risken för
        // false positives är låg eftersom kravet på en av lagspråk-
        // signalerna ovan (förenlig-ord, DIGG-länk, eller 12 §) redan
        // är ett starkt filter.
        let h1 = body.querySelector('h1');
        if (h1) {
            knowledgeData['h1'] = h1.textContent.replace(/\u00AD/g, '').trim();
            const isA11yStatementH1 = /tillg(.{1,6}|ä|&auml;|&#228;)nglighet/gim.test(knowledgeData['h1']);
        ...
        if (title) {
            ...
            const isA11yStatementTitle = /tillg(.{1,6}|ä|&auml;|&#228;)nglighet/gim.test(knowledgeData['page-title']);
